...
class ASFF(nn.Module):
    def __init__(self, level, expansion, rfb=False, vis=False):
        ...
        self.SE_level0 = ECALayer(self.inter_dim)
        self.SE_level1 = ECALayer(self.inter_dim)
        self.SE_level2 = ECALayer(self.inter_dim)
...
class ResNet(nn.Module):
  def __init__(self, block, layers, num_classes=2):
    ...
    self.ECA_layer4 = ECALayer(512*block.expansion)

    self.fusion = ASFF(level=2, expansion= block.expansion, rfb=False, vis=False)
    self.ECA_fusion = ECALayer(192 * block.expansion)
